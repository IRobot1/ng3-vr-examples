...
export default class NumberController extends Controller {
  ...
  override decimals(decimals: number): Controller {
    this._decimals = decimals;
    this.updateDisplay();
    return this;
  }

  min(min: number): NumberController {
    this._min = min;
    this._onUpdateMinMax();
    return this;
  }

  max(max: number): NumberController {
    this._max = max;
    this._onUpdateMinMax();
    return this;
  }

  step(step?: number): NumberController {
    if (step) {
      this._step = step;
  ...
  _onUpdateMinMax() {
    ...
    if (!this._hasSlider && this._hasMin && this._hasMax) {
      ...
      // update that too.
      if (!this._stepExplicit) {
        this.step(this._getImplicitStep());
      }
